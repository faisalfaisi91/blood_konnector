 ...
 */
function lifelineAutoAssignDonor($conn, $recipientId, $bloodTypeOverride = null, $city = null) {
...
/**
 * Find multiple matching donors and notify them about a lifeline request.
 * Returns array of donor IDs that were notified.
 */
function lifelineNotifyMatchingDonors($conn, $requestId, $recipientId, $bloodTypeOverride = null, $city = '', $scheduledAt = '', $excludeDonorId = null) {
    ...
    if (empty($blood)) {
        $reqStmt = $conn->prepare("SELECT blood_type FROM lifeline_requests WHERE id = ? LIMIT 1");
    ...
    if (empty($matchingCity)) {
        $cityStmt = $conn->prepare("SELECT city FROM lifeline_requests WHERE id = ? LIMIT 1");
    ...
    $query = "
        ...
        WHERE d.blood_type = ?
          AND d.user_id != ?
          AND d.user_id NOT IN (SELECT donor_id FROM lifeline_confirmations WHERE request_id = ? AND donor_id IS NOT NULL)
        ORDER BY 
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        if ($checkResult && $checkResult->num_rows > 0) {
            ...
            $notifStmt = $conn->prepare("INSERT INTO lifeline_notifications (user_id, channel, template_key, payload, status) VALUES (?, 'in_app', 'lifeline_new_request', ?, 'queued')");
...
switch ($action) {
    case 'create_request':
        ...
        if (!$donorId) {
            $auto = lifelineAutoAssignDonor($conn, $userId, $matchingBloodType, $city);
            if ($auto) {
                $donorId = $auto;
            }
        }
        ...
        $notifiedCount = 0;
        if (!$donorId || $donorId === '') {
            $notifiedDonors = lifelineNotifyMatchingDonors($conn, $requestId, $userId, $bloodTypeOverride, $city, $scheduledAt, $donorId);
    ...
    case 'donor_response':
        ...
        try {
            ...
            if ($response === 'approve') {
                ...
                $notifStmt = $conn->prepare("INSERT INTO lifeline_notifications (user_id, channel, template_key, payload, status) VALUES (?, 'in_app', 'lifeline_donor_approved', ?, 'queued')");
    ...
    case 'get_unread_count':
        $countStmt = $conn->prepare("SELECT COUNT(*) as cnt FROM lifeline_notifications WHERE user_id = ? AND channel = 'in_app' AND status = 'queued' AND template_key IN ('lifeline_new_request', 'lifeline_donor_approved')");
    ...
    case 'get_notifications':
        // Fetch lifeline notifications for current user
        $notifications = [];
