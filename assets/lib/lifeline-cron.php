...
$due = $conn->prepare("
    ...
    FROM lifeline_reminders r
    JOIN lifeline_requests lr ON lr.id = r.request_id
...
while ($row = $result->fetch_assoc()) {
    ...
    foreach ($users as $uid) {
        $stmt = $conn->prepare("INSERT INTO lifeline_notifications (user_id, channel, template_key, payload, status) VALUES (?, 'in_app', ?, ?, 'queued')");
        $payload = json_encode([
            'request_id' => $row['id'],
        ...
        foreach ($channels as $ch) {
            $s2 = $conn->prepare("INSERT INTO lifeline_notifications (user_id, channel, template_key, payload, status) VALUES (?, ?, ?, ?, 'queued')");
            $s2->bind_param("ssss", $uid, $ch, $template, $payload);
            $s2->execute();
            $s2->close();
        }
    }
    $mark = $conn->prepare("UPDATE lifeline_reminders SET sent_at = NOW() WHERE id = ?");
